...
-- for a given Haskell program, which we will call the profilee.
-- It also provides our default set of perf options and events to be recorded.
--
-- First it forks a process for the profilee, then it forks
-- another process which attaches "perf record"
-- onto the PID of the profilee. It then waits for the
...
-- This tool runs "perf", adding our default set
...
-- the command line.
-- TODO:
--   - wait util -p in  perf-record is fixed so that it records from all
--     threads of a given process, no just the threads spawned before perf
--     is started
...
--
-- Note: an alternative to -p suggested on linux-perf-users@vger.kernel.org
-- is to use -G as follows:
...
waitForProcessTerminate pid = do
   ...
   case processStatus of
      ...
      Just (Exited _code) -> return ()
      Just (Terminated _signal) -> return ()
      Just (Stopped _signal) ->
         -- XXX what to do here?
         return ()
...
profileeProcess options program args = do
   -- wait for a short time to allow perf to start recording this process
   threadDelay $ options_wait options
   -- this one does not work, because it's optimized away, but we should
   -- call nanosleep in a similar, direct way to avoid overhead, etc.
   t <- getTimeOfDay
   -- debug
   print t
   -- call nanosleep to synchronise times
   nanosleep 3
...
perfProcess options pid = do
   ...
   where
   perfCommand = ["record"]
   args = concat [output, frequency, moreTimestamps, mmap, attachPID, selectedEvents, dummyCommand]
